...
<body
    class="bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-gray-100 h-screen w-full flex items-center justify-center sm:p-6 transition-colors duration-300">

    <!-- Welcome Loader Screen -->
    <div id="welcome-loader"
        class="fixed inset-0 bg-gradient-to-br from-brand-600 via-indigo-600 to-purple-700 flex items-center justify-center z-[100]">
        <div class="text-center px-6 max-w-md">
            <!-- Logo -->
            <div class="logo-animate mb-8">
                @if(system_setting('app_favicon'))
                    <img src="{{ asset('storage/' . system_setting('app_favicon')) }}"
                        alt="{{ system_setting('app_name', 'Logo') }}"
                        class="w-24 h-24 mx-auto rounded-3xl shadow-2xl ring-4 ring-white/20 object-cover">
                @else
                    <div
                        class="w-24 h-24 mx-auto rounded-3xl bg-white/10 backdrop-blur-sm flex items-center justify-center shadow-2xl ring-4 ring-white/20">
            ...
            <div class="text-animate space-y-4">
                ...
                <p class="text-lg text-white/90 mb-6">{{ system_setting('app_tagline', 'Your AI-Powered Assistant') }}
                </p>
                <p class="text-sm text-white/80 leading-relaxed mb-8">
                    Experience intelligent conversations powered by advanced AI technology. Get instant answers,
    ...
    <!-- Mode Selector Modal -->
    <div id="mode-selector" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50"
        style="display:none;">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 max-w-xs w-full text-center">
            ...
            <button
                class="mode-btn w-full mb-3 px-4 py-2 rounded-lg bg-brand-600 text-white font-semibold hover:bg-brand-700 transition"
                data-mode="default">Standard Mode</button>
            <button
                class="mode-btn w-full px-4 py-2 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition"
                data-mode="research">Research Mode</button>
        </div>
    </div>
    ...
    <div id="chat-window"
        class="w-full h-full sm:max-w-[480px] md:max-w-[700px] lg:max-w-[900px] sm:h-[85vh] bg-white dark:bg-gray-800 sm:rounded-2xl sm:shadow-2xl border-0 sm:border border-gray-100 dark:border-gray-700 flex flex-col overflow-hidden relative">

        <!-- Header -->
        <div
            class="px-6 py-4 bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between fixed sm:sticky top-0 left-0 right-0 sm:left-auto sm:right-auto w-full z-20">
            <div class="flex items-center space-x-4">
                <div class="relative">
                    ...
                    @else
                        <div
                            class="w-10 h-10 rounded-full bg-gradient-to-tr from-brand-500 to-indigo-600 flex items-center justify-center text-white shadow-md ring-2 ring-white dark:ring-gray-800">
                ...
                <div>
                    <span
                        class="text-lg font-bold tracking-tight text-gray-900 dark:text-white">{{ system_setting('app_name', 'Premium Chatbot') }}</span>
                    <span
                        class="block text-xs text-gray-500 dark:text-gray-400">{{ system_setting('app_tagline', 'Your AI Assistant') }}</span>
                </div>
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <button id="clear-chat-btn"
                    class="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900 text-red-500 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 transition-colors"
                    title="Clear Chat & Start New">
                    <i data-lucide="refresh-ccw" class="w-5 h-5"></i>
                </button>
                <!-- Interest Selector Dropdown -->
                <div class="relative">
                    <button id="interest-toggle"
                        class="p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 transition-colors"
                        title="Select Interest">
                        <i data-lucide="filter" class="w-5 h-5"></i>
                    </button>

                    <!-- Dropdown Menu -->
                    <div id="interest-dropdown"
                        class="absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-100 dark:border-gray-700 py-1 z-20 hidden">
                        <div class="px-3 py-2 border-b border-gray-100 dark:border-gray-700">
                            <p class="text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                                Select Interest</p>
                        </div>
                        <button onclick="selectInterest('')"
                            class="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center justify-between">
                            <span>All Interests</span>
                            <i data-lucide="check" class="w-4 h-4 text-brand-600" id="check-all"
                                style="display: none;"></i>
                        </button>
                        <button onclick="selectInterest('general')"
                            class="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center justify-between">
                            <span>General</span>
                            <i data-lucide="check" class="w-4 h-4 text-brand-600" id="check-general"
                                style="display: none;"></i>
                        </button>
                        <button onclick="selectInterest('technical')"
                            class="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center justify-between">
                            <span>Technical</span>
                            <i data-lucide="check" class="w-4 h-4 text-brand-600" id="check-technical"
                                style="display: none;"></i>
                        </button>
                        <button onclick="selectInterest('business')"
                            class="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center justify-between">
                            <span>Business</span>
                            <i data-lucide="check" class="w-4 h-4 text-brand-600" id="check-business"
                                style="display: none;"></i>
                        </button>
                        <button onclick="selectInterest('medical')"
                            class="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center justify-between">
                            <span>Medical</span>
                            <i data-lucide="check" class="w-4 h-4 text-brand-600" id="check-medical"
                                style="display: none;"></i>
                        </button>
                        <button onclick="selectInterest('legal')"
                            class="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center justify-between">
                            <span>Legal</span>
                            <i data-lucide="check" class="w-4 h-4 text-brand-600" id="check-legal"
                                style="display: none;"></i>
                        </button>
                        <button onclick="selectInterest('educational')"
                            class="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center justify-between">
                            <span>Educational</span>
                            <i data-lucide="check" class="w-4 h-4 text-brand-600" id="check-educational"
                                style="display: none;"></i>
                        </button>
                        <button onclick="selectInterest('research')"
                            class="w-full text-left px-3 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700 flex items-center justify-between">
                            <span>Research</span>
                            <i data-lucide="check" class="w-4 h-4 text-brand-600" id="check-research"
                                style="display: none;"></i>
                        </button>
                    </div>
                </div>
                ...
                @auth('admin')
                    <a href="{{ route('admin.dashboard') }}"
                        class="p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 transition-colors"
                        title="Admin Panel" target="_blank">
        ...
        <div id="messages-container"
            class="flex-1 overflow-y-auto custom-scrollbar p-4 sm:p-6 space-y-6 bg-gray-50/50 dark:bg-gray-900/50 pt-[88px] sm:pt-6 pb-[120px] sm:pb-6">

            <!-- Date Separator -->
            <div class="flex justify-center my-4">
                <span
                    class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider bg-gray-100 dark:bg-gray-800 px-3 py-1 rounded-full">Today</span>
            </div>

            <!-- Bot Welcome Message (Static) -->
            <div class="flex items-start space-x-3 message-enter max-w-3xl mx-auto w-full">
                <div
                    class="w-8 h-8 rounded-full bg-brand-100 dark:bg-brand-900/30 flex-shrink-0 flex items-center justify-center text-brand-600 dark:text-brand-400 mt-1">
                    <i data-lucide="sparkles" class="w-4 h-4"></i>
                </div>
                <div class="space-y-1 flex-1">
                    <div
                        class="bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-2xl rounded-tl-none px-5 py-4 shadow-sm w-fit max-w-[90%] sm:max-w-[80%]">
                        <p class="text-sm text-gray-700 dark:text-gray-200 leading-relaxed">
                            Hello! 👋 I'm your dedicated virtual assistant. <br><br>
                            I can help you analyze documents, manage workflows, or answer complex support questions. How
        ...
        <div id="typing-indicator" class="hidden px-6 pb-2 pt-0 bg-gray-50/50 dark:bg-gray-900/50">
            <div class="max-w-3xl mx-auto w-full flex items-start space-x-3">
                ...
                <div
                    class="bg-gray-200 dark:bg-gray-700 rounded-2xl rounded-tl-none px-4 py-3 w-16 flex items-center justify-center space-x-1">
        ...
        <!-- Input Area -->
        <div
            class="p-4 sm:p-6 bg-white dark:bg-gray-800 border-t border-gray-100 dark:border-gray-700 fixed sm:relative bottom-0 left-0 right-0 sm:left-auto sm:right-auto w-full z-20">
            <div class="max-w-3xl mx-auto w-full">
                <form id="chat-form"
                    class="relative flex items-end shadow-sm rounded-xl bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-600 focus-within:ring-2 focus-within:ring-brand-500/20 focus-within:border-brand-500 transition-all">
                    <input type="text" id="chat-input" placeholder="Ask anything about your documents..."
                ...
                <div class="text-center mt-3 flex items-center justify-center space-x-2">
                    ...
                    <p class="text-[10px] text-gray-400">Secure Enterprise Environment • <a href="#"
                            class="hover:underline hover:text-brand-500 transition-colors">Privacy Policy</a></p>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#clear-chat-btn').on('click', function () {
                // Remove session id and interest
                localStorage.removeItem('chatbot-session-id');
                localStorage.removeItem('chatbot-interest');
                // Optionally clear chatMode if used
